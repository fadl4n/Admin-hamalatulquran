...
class HistoriController extends Controller
{
    ...
    public function fnGetData(Request $request)
    {
        ...
        return datatables()->eloquent($query)
            ...
            ->addColumn('status', function ($histori) {
                $target = $histori->targets;
                if (!$target) return '-';

                $totalAyat = $target->setoran()->sum(DB::raw('jumlah_ayat_end - jumlah_ayat_start + 1'));
                $jumlahTarget = $target->jumlah_ayat_target;
                $tglTarget = $target->tgl_target;
                $tglSetoranTerakhir = $target->setoran()->max('tgl_setoran');

                return Histori::determineStatus($totalAyat, $jumlahTarget, $tglTarget, $tglSetoranTerakhir);
            })
